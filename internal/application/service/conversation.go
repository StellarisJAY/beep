...
func (c *ConversationService) List(ctx context.Context, query types.ConversationQuery) ([]*types.Conversation, error) {
	query.UserId = ctx.Value(types.UserIdContextKey).(string)
...
func (c *ConversationService) ListMessages(ctx context.Context, conversationId string) ([]*types.Message, error) {
