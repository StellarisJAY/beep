...
type MCPServerRepo interface {
	...
	Get(ctx context.Context, id string) (*types.MCPServer, error)
}
...
type MCPServerService interface {
	...
	// Call 调用 MCP 服务器工具
	Call(ctx context.Context, id string, request *mcp.CallToolParams) (*mcp.CallToolResult, error)
	CallWithElicitation(ctx context.Context, id string, request *mcp.CallToolParams) (*mcp.CallToolResult, error)
}
